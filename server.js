  ...
  try {
    ...
    const row = new ActionRowBuilder()
      .addComponents(
        ...
        new ButtonBuilder()
          .setCustomId('get_asset_script_from_log')
...
async function handleBlacklist(interaction) {
  try {
    ...
    // CORRECTED REGEX FOR DISCORD ID
    ...
    if (!discordIdMatch || !discordIdMatch[1]) {
      console.log("Failed to match Discord ID. Embed description for debugging:", embed.description); // Log for debugging
    ...
    try {
      ...
      if (guild) {
        ...
        if (member) {
          ...
          if (rolesActuallyRemoved.length > 0) rolesRemovedMessage = `Removed roles: ${rolesActuallyRemoved.join(', ')}.`;
    ...
    try {
      ...
      const blacklistDmEmbed = new EmbedBuilder()
        ...
        ).setTimestamp().setFooter({ text: 'Contact support if you believe this is an error.' });
    ...
    if (logChannel) {
      const logEmbed = new EmbedBuilder()
        ...
        ).setTimestamp().setFooter({ text: 'Blacklist System Log' });
...
async function handleGetAssetOrScript(interaction) {
  try {
    ...
    const getObjectsRegex = /(?:GetObjects|InsertService:LoadAsset(?:Version)?)\s*\(\s*(?:["']rbxassetid:\/\/(\d+)["']|(\d+))\s*\)/gi;
    const contentRegex = /Content\s*=\s*["']rbxassetid:\/\/(\d+)["']/gi;
    const imageRegex = /Image\s*=\s*["']rbxassetid:\/\/(\d+)["']/gi;
    const textureRegex = /Texture(?:Id)?\s*=\s*["']rbxassetid:\/\/(\d+)["']/gi;
    const soundRegex = /SoundId\s*=\s*["']rbxassetid:\/\/(\d+)["']/gi;
    const meshRegex = /MeshId\s*=\s*["']rbxassetid:\/\/(\d+)["']/gi;


    let match;
    while ((match = requireRegex.exec(scriptContentToAnalyze)) !== null) assetIds.add(match[1]);
    while ((match = getObjectsRegex.exec(scriptContentToAnalyze)) !== null) assetIds.add(match[1] || match[2]); // match[1] for rbxassetid, match[2] for direct ID
    while ((match = contentRegex.exec(scriptContentToAnalyze)) !== null) assetIds.add(match[1]);
    while ((match = imageRegex.exec(scriptContentToAnalyze)) !== null) assetIds.add(match[1]);
    while ((match = textureRegex.exec(scriptContentToAnalyze)) !== null) assetIds.add(match[1]);
    while ((match = soundRegex.exec(scriptContentToAnalyze)) !== null) assetIds.add(match[1]);
    while ((match = meshRegex.exec(scriptContentToAnalyze)) !== null) assetIds.add(match[1]);
    ...
    if (assetIds.size > 0) {
      const uniqueAssetIds = Array.from(assetIds).filter(id => id); // Filter out undefined if regex captures optional groups
      if (uniqueAssetIds.length > 0) {
        const assetLinks = uniqueAssetIds.map(id => `[${id}](https://www.roblox.com/library/${id})`).join('\n');
        replyContent = `Found Asset ID(s) in script:\n${assetLinks}\n\nThe full script is also attached.`;
      }
    }

    await interaction.editReply({
      content: replyContent,
      files: attachmentsForReply,
      ephemeral: true
    });
...
app.get('/download/:assetId', async (req, res) => {
  try {
    ...
    const fileName = `${assetId}.rbxm`; // It's still a placeholder
    const content = `-- Roblox model reference (AssetId: ${assetId})\n-- To get the actual asset, use the ID on the Roblox website or in Studio.`;
...
app.get('/scripts/LuaMenu', async (req, res) => {
  ...
  try {
    const response = await axios.get(config.GITHUB_LUA_MENU_URL, {
      timeout: 8000,
      headers: { 'User-Agent': 'LuaWhitelistServer/1.4' }
    });
    res.set({
